...
export const getAttempt = catchAsync(async (req, res) => {
  const result = await ReviewService.getReviewAttempt(req.user!.id, req.params.attemptId);
  sendResponse(res, 200, "Review attempt retrieved successfully", result);
});

export const evaluate = catchAsync(async (req, res) => {
  const result = await ReviewService.evaluate(req.user!.id, req.params.attemptId, req.body);
