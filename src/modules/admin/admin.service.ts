import { AssessmentStatus, AttemptStatus, type Prisma, Role, UserStatus } from "@prisma/client";
...
export const updateUserStatus = async (adminId: string, userId: string, status: UserStatus) => {
  ...
  const updated = await prisma.user.update({
    ...
    data: {
      status,
      ...(status === UserStatus.BLOCKED ? { tokenVersion: { increment: 1 } } : {}),
    },
    select: {
      id: true,
      name: true,
...
export const updateUserRole = async (adminId: string, userId: string, role: Role) => {
  ...
  const activeWork = await prisma.attempt.count({
    where: {
      ...
      status: {
        in: [AttemptStatus.IN_PROGRESS, AttemptStatus.SUBMITTED, AttemptStatus.UNDER_REVIEW],
      },
    },
  });
...
export const stats = async () => {
  const [users, candidates, reviewers, publishedAssessments, attempts, evaluated, payments] =
